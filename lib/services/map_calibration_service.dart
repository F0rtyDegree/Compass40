...
class MapCalibrationService {
  ...
  void toggleAnchorPinned(String anchorId) {
    ...
    if (_pinnedAnchorIds.contains(anchorId)) {
      ...
      //
      // Таймер не отменяется. Осознанно оставлено как есть.
      // Обоснование: событие редкое (быстрые множественные клики маловероятны),
      // при закрытии экрана карты вызов отработает на объекте, который уже
      // не в UI, и безвредно завершится. Возвращаться к этому — только если
      // появятся реальные проблемы.
  ...
  /// в _anchors) стоит первым — но только если он входит в points.
  /// Если его там нет, первым становится первый элемент из points.
  ///
  /// В ручном режиме пользователь сам выбирает, какие точки важны,
  /// и порядок для веса не критичен. Осознанное упрощение.
